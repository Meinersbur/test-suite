...
macro(llvm_singlesource)
  ...
  foreach(source ${sources})
    ...
    if(NOT TESTSCRIPT)
      llvm_test_traditional(${name} PREFIX "${_LSARG_PREFIX}")
      llvm_test_prefix(PREFIX ${_LSARG_PREFIX})
      #llvm_test_sources(${source})
    endif()
...
function(llvm_test_traditional name)
  ...
  if(NOT PROGRAM_IS_NONDETERMINISTIC)
    ...
    if(NOT NO_REFERENCE_OUTPUT)
      ...
      elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${ENDIAN}-endian.${KEY})
        set(REFERENCE_OUTPUT ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${ENDIAN}-endian.${KEY})
      elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${KEY})
        set(REFERENCE_OUTPUT ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${KEY})
      elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${ENDIAN}-endian)
        set(REFERENCE_OUTPUT ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output.${ENDIAN}-endian)
      elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output)
        set(REFERENCE_OUTPUT ${CMAKE_CURRENT_SOURCE_DIR}/${prefix}${name}.reference_output)
        
      else()
        message("-- No reference output found for test ${prefix}${name}")
      endif()
    endif()
    ...
    llvm_test_refout("${REFERENCE_OUTPUT}")
  endif()
